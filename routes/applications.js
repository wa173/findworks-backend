        ...
        <div style="background:#fff;padding:28px 24px;border-radius:0 0 12px 12px;border:1px solid #e5e7eb;border-top:none;">
          ...
          <div style="background:#f0fdf4;border:1px solid #bbf7d0;border-radius:8px;padding:16px;margin:20px 0;">
            <p style="margin:0;color:#166534;font-size:14px;line-height:1.6;">
              ✅ Great news! The client is interested in your profile. Check your WhatsApp for any messages from them, or log in to FindWorks to view their contact details.
            </p>
          </div>` : `
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      if (job) {
        // Get client email from users table if not on job
...
router.patch('/:id', async (req, res) => {
  ...
  try {
    ...
    // Send email to worker when status changes
    try {
